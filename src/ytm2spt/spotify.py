...
class Spotify:
    ...
    def set_playlist_description(
        ...
        self.spotify_logger.debug(
            f"Set Playlist {playlist_id} Description {description}"
        )
    ...
    def get_song_uri(self, artist: str, song_name: str) -> Union["str", None]:
        try:
            results = self.spotify.search(
                f"{song_name} {artist}", type="track", limit=10
            )
    ...
    def add_song_to_playlist(self, song_uri: str, playlist_id: str = "") -> bool:
        ...
        try:
            ...
            self.spotify_logger.debug(
                f"Added Song {song_uri} to Playlist {playlist_id}"
            )
            return True
        except SpotifyException as e:
